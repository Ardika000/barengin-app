...
// Nb: Toaster dirender sebagai sibling dari <App> di app.jsx (di luar konteks
// Inertia), jadi tidak bisa memakai usePage()/useTranslation() di sini.
export default function Toaster() {
    ...
    return (
        <div className="fixed top-25 inset-x-3 sm:inset-x-auto sm:right-5 z-[10000] flex flex-col items-center sm:items-end gap-2.5 font-poppins pointer-events-none">
            {items.map((toastItem) => {
                const v = VARIANTS[toastItem.type] || VARIANTS.info;
                const { Icon } = v;
                return (
                    <div
                        key={toastItem.id}
